...
namespace Bodega
{
    public partial class Bodeguero : Form
    {
            ...
            if (comboBoxP.SelectedIndex >= 0)
            {
                ...
                try
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        // Limpia los campos después de la inserción si es necesario
        ...
        private int ObtenerIdProveedorDeProducto(object productoId)
        {
            ...
            using (OracleConnection connection = db.Conectar())
            {
                ...
                try
                {
                    ...
                    {
                        // Si no se encuentra el id_proveedor, puedes manejarlo de acuerdo a tu lógica.
                        // Puedes lanzar una excepción, devolver un valor predeterminado u otro manejo.
        ...
        private void EliminarSolicitud(int idSolicitud)
        {
            try
            {
                using (OracleConnection connection = db.Conectar()) // Utiliza tu conexión existente
                {
        ...
        private void dataGridView2_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0 && dataGridViewSolicitudes.Columns[e.ColumnIndex].Name == "Eliminar")
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    // LlenarDataGridViewSolicitudes(); // o el nombre que corresponda
                }
            }
        }
        ...
        private void comboBox1_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            ...
            if (comboBox1.SelectedIndex >= 0)
            {
                ...
                // Obtén los datos de la receta seleccionada desde Oracle y llénalos en los TextBox
                ...
                using (OracleConnection connection = db.Conectar()) // Utiliza tu conexión existente
                {
                    using (OracleCommand cmd = new OracleCommand("DELETE FROM RECETA WHERE ID_RECETA = :idReceta", connection))
                    {
                        ...
                        if (rowsAffected > 0)
                        {
                            ...
                            // Actualiza el DataGridView de recetas (si tienes un método para ello)
                            // LlenarDataGridViewRecetas(); // o el nombre que corresponda
                        }
                        else
                        {
        ...
        {
            // Asegúrate de que el clic fue en la columna del botón "Eliminar" y no en el encabezado
            if (e.RowIndex >= 0 && dataGridView3.Columns[e.ColumnIndex].Name == "BorrarReceta")
            {
                // Obtén el ID de la receta de la fila seleccionada
