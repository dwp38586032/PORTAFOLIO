...
namespace WindowsFormsApp2
{
    public partial class Administrador : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                // Vuelve a llenar el DataSet con datos frescos de la tabla 'USUARIO'
                this.uSUARIOTableAdapter.Fill(this.usuarios.USUARIO);
            }
            catch (Exception ex)
            {
        ...
        private void dataGridView2_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                // Actualizar DataGridView (si tienes un método para ello)
                // LlenarDataGridView3(); // o el nombre que corresponda
            }
        }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                this.mESATableAdapter.Fill(this.mesas.MESA); // Reemplaza 'tuDataSet' con el nombre de tu DataSet.
        ...
        {
            // Asegúrate de que el clic fue en la columna del botón de eliminar y no en el encabezado
            if (e.RowIndex < 0 || e.ColumnIndex != dataGridView5.Columns["Borrar"].Index) return;

            // Obtener el ID del cliente de la fila seleccionada
            int clienteId = Convert.ToInt32(dataGridView5.Rows[e.RowIndex].Cells["ID_CLIENTE"].Value); // Asegúrate de que "ID_CLIENTE" es el nombre correcto de la columna en tu DataGridView.
            ...
            if (result == DialogResult.Yes)
            {
                // Llama a tu método para eliminar el cliente de la base de datos
                BDoracle bd = new BDoracle();
                bd.BorrarCliente(clienteId); // Asume que tienes un método llamado 'BorrarCliente' que elimina un cliente por ID.

                // Suponiendo que tienes un adaptador y un método para actualizar la lista de clientes después de la eliminación.
                // Reemplaza 'tuDataSet' y 'cLIENTETableAdapter' con los nombres correctos si son diferentes.
                this.cLIENTETableAdapter.Fill(this.cliente.CLIENTE);
                dataGridView5.DataSource = cLIENTEBindingSource;

                // Refresca el DataGridView si tienes un método para ello.
            }
        }
        ...
        private void comboBox1_SelectedIndexChanged_3(object sender, EventArgs e)
        {
                ...
                if (comboBoxCliente.SelectedIndex >= 0)
                {
                    ...
                    // Obtén los datos del producto seleccionado desde Oracle y llénalos en los TextBox
        ...
        {
            // When the Modify button is pressed, update the data in Oracle
            if (comboBoxCliente.SelectedIndex >= 0)
            {
                object selectedValue = comboBoxCliente.SelectedValue;
                ...
                try
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        // Assuming there's a method or adapter to refresh client data after update, similar to PRODUCTOTableAdapter for products
                        try
                        {
                            this.cLIENTETableAdapter.Fill(this.cliente.CLIENTE);
                        }
        ...
        {
            // Asegúrate de que el clic fue en la columna del botón de eliminar y no en el encabezado
            if (e.RowIndex < 0 || e.ColumnIndex != dataGridView5.Columns["EliminarCliente"].Index) return;

            // Obtener el ID del cliente de la fila seleccionada
            int clienteId = Convert.ToInt32(dataGridView5.Rows[e.RowIndex].Cells["iDCLIENTEDataGridViewTextBoxColumn"].Value); // Asegúrate de que "ID_CLIENTE" es el nombre correcto de la columna en tu DataGridView.
