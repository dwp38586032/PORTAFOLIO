...
namespace WindowsFormsApp2
{
    public class BDoracle
    {
        ...
        public void BorrarCliente(int idCliente)
        {
            ...
            {
                conexion.Open();
                OracleCommand cmd = new OracleCommand("DELETE FROM CLIENTE WHERE ID_CLIENTE = :clienteId", conexion); // Nota el cambio aquí
                cmd.Parameters.Add(new OracleParameter(":clienteId", idCliente)); // Asegúrate de agregar el ':' antes del nombre del parámetro
                cmd.ExecuteNonQuery();
            }
        }
